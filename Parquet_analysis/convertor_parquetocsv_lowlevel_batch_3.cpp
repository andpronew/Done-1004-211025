...
// Escape CSV fields with comma, quote, newline; double internal quotes
...
int parquet_to_csv_lowlevel_batch(const string &input_file, const string &output_file, int batch_size = 1024)
{
    try
    {
        ...
        for (int rg_idx = 0; rg_idx < num_row_groups; ++rg_idx) {
            ...
            vector<shared_ptr<parquet::ColumnReader>> col_readers(num_columns);
            for (int c = 0; c < num_columns; ++c) {
                col_readers[c] = rg_reader->Column(c);
            }

            // Allocate buffers for definition and repetition levels
            ...
            int64_t row_pos = 0;
            while (row_pos < rows_in_rg) {
                int64_t rows_to_read = min<int64_t>(batch_size, rows_in_rg - row_pos);

                // Resize buffers for batch
                for (int c = 0; c < num_columns; ++c) {
                ...
                for (int c = 0; c < num_columns; ++c) {
                    ...
                    switch (phys) {
                        case parquet::Type::INT32: {
                            auto r = static_cast<parquet::Int32Reader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), int32_buffers[c].data());
                            break;
                        }
                        case parquet::Type::INT64: {
                            auto r = static_cast<parquet::Int64Reader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), int64_buffers[c].data());
                            break;
                        }
                        case parquet::Type::FLOAT: {
                            auto r = static_cast<parquet::FloatReader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), float_buffers[c].data());
                            break;
                        }
                        case parquet::Type::DOUBLE: {
                            auto r = static_cast<parquet::DoubleReader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), double_buffers[c].data());
                            break;
                        }
                        case parquet::Type::BOOLEAN: {
                            auto r = static_cast<parquet::BoolReader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), bool_buffers[c].data());
                            break;
                        }
                        case parquet::Type::BYTE_ARRAY: {
                            auto r = static_cast<parquet::ByteArrayReader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), ba_buffers[c].data());
                            break;
                        }
                        case parquet::Type::FIXED_LEN_BYTE_ARRAY: {
                            auto r = static_cast<parquet::FixedLenByteArrayReader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), flba_buffers[c].data());
                            break;
                        }
                        case parquet::Type::INT96: {
                            auto r = static_cast<parquet::Int96Reader*>(col_readers[c].get());
                            values_read[c] = r->ReadBatch(rows_to_read,
                                def_levels[c].data(), rep_levels[c].data(), int96_buffers[c].data());
                            break;
                        }
                        default:
                            values_read[c] = 0; // unsupported
                            break;
                    }
                }

                // Write CSV lines for batch rows
                        ...
                        if (def_levels[c][i] < max_def_level) {
                            ...
                            switch (phys) {
                                ...
                                case parquet::Type::INT64:
                                    ...
                                    break;
                                case parquet::Type::FLOAT: {
                                    ostringstream oss; oss << float_buffers[c][i];
                                    cell_text = oss.str();
                                    break;
                                }
                                case parquet::Type::DOUBLE: {
                                    ostringstream oss; oss << double_buffers[c][i];
                                    cell_text = oss.str();
                                    break;
                                }
                                case parquet::Type::BOOLEAN:
