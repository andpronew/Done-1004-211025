...
// Convert INT96 (legacy timestamp) to hex string (datetime conversion is complex)
...
// Escape CSV field if contains comma, quote or newline, doubling quotes inside
...
int parquet_to_csv_lowlevel_batch(const string &input_file, const string &output_file)
{
    try
    {
        ...
        constexpr int64_t batch_size = 1024; // read rows in batches
        ...
        for (int rg_idx = 0; rg_idx < num_row_groups; ++rg_idx) {
            ...
            // Read row group in batches
            for (int64_t offset = 0; offset < rows_in_rg; offset += batch_size) {
                int64_t rows_to_read = std::min(batch_size, rows_in_rg - offset);

                // For each column, prepare buffer for batch read
                ...
                vector<vector<uint8_t>> bool_buffers(num_columns);
                ...
                // Prepare definition and repetition level buffers for batch reading
                vector<vector<int16_t>> def_levels(num_columns, vector<int16_t>(rows_to_read));
                vector<vector<int16_t>> rep_levels(num_columns, vector<int16_t>(rows_to_read));

                // Keep track of how many values were read per column
                vector<int64_t> values_read(num_columns, 0);

                // Batch-read each column
                for (int c = 0; c < num_columns; ++c) {
                    ...
                    switch (phys) {
                        case parquet::Type::INT32: {
                            ...
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), int32_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::INT64: {
                            int64_buffers[c].resize(rows_to_read);
                            parquet::Int64Reader* r = static_cast<parquet::Int64Reader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), int64_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::FLOAT: {
                            float_buffers[c].resize(rows_to_read);
                            parquet::FloatReader* r = static_cast<parquet::FloatReader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), float_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::DOUBLE: {
                            double_buffers[c].resize(rows_to_read);
                            parquet::DoubleReader* r = static_cast<parquet::DoubleReader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), double_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::BOOLEAN: {
                            bool_buffers[c].resize(rows_to_read);
                            parquet::BoolReader* r = static_cast<parquet::BoolReader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), bool_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::BYTE_ARRAY: {
                            ba_buffers[c].resize(rows_to_read);
                            parquet::ByteArrayReader* r = static_cast<parquet::ByteArrayReader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), ba_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::FIXED_LEN_BYTE_ARRAY: {
                            ...
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), flba_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        case parquet::Type::INT96: {
                            int96_buffers[c].resize(rows_to_read);
                            parquet::Int96Reader* r = static_cast<parquet::Int96Reader*>(col_readers[c].get());
                            r->ReadBatch(rows_to_read, def_levels[c].data(), rep_levels[c].data(), int96_buffers[c].data(), &values_read[c]);
                            break;
                        }
                        default:
                            // Unsupported physical type: skip reading
                            values_read[c] = 0;
                            break;
                    }
                }

                // Now write out rows for this batch
                for (int64_t i = 0; i < rows_to_read; ++i) {
                    for (int c = 0; c < num_columns; ++c) {
                        ...
                        // If definition level < max_def_level, the value is NULL / missing
                        if (def_levels[c][i] < max_def_level) {
                            ...
                            switch (phys) {
                                ...
                                case parquet::Type::FIXED_LEN_BYTE_ARRAY: {
                                    ...
                                    cell_text = FixedLenToHex(flba_buffers[c][i].ptr, len);
                                    break;
                                }
                                case parquet::Type::INT96:
